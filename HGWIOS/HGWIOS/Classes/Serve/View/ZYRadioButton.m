...
-(id)initWithGroupId:(NSString*)groupId index:(NSUInteger)index selectedImageName:(NSString *)selectedImage unSelectedImageName:(NSString *)unselectedImage{
    selectedImageName = selectedImage;
    unSelectedImageName = unselectedImage;
    //调用初始化函数
    self = [self init];
    if (self) {
        _groupId = groupId;
...
-(void)defaultInit{
    ...
    //正常状态下的按钮背景图片
    if (unSelectedImageName ==NULL ||unSelectedImageName.length == 0) {
        [_button setImage:[UIImage imageNamed:@"RadioButton-Unselected"] forState:UIControlStateNormal];
    }else{
    [_button setImage:[UIImage imageNamed:unSelectedImageName] forState:UIControlStateNormal];
    }
       //选择状态下的按钮背景图片
    if (selectedImageName ==NULL ||selectedImageName.length == 0) {
       ...
       [_button setImage:[UIImage imageNamed:selectedImageName] forState:UIControlStateSelected];
    }
