...
@property (nonatomic, weak) ShaiXuanView * shaiView;
...
//空载港参数
@property (nonatomic, copy) NSString * n_portID;
...
//    [self newGuide];
    ...
    SendGoodsViewController * sendVc = [[SendGoodsViewController alloc]init];
...
-(void)shuaiXuanView
{
    ...
    shaiView.shaiChooseBlock = ^(NSString *str, NSString *weightStr, NSString *cashStr, NSString *payStr, NSString *typeStr) {
        ...
        if ([str isEqualToString:@"确 定"]) {
            ...
            weakSelf.typeStr = [typeStr isEqualToString:@"不限"] || typeStr == nil ? @"" : typeStr;
    ...
    [shaiView setFromTag:@"找船" Array:self.shaiXuanArray];
    ...
    topView.topblock = ^(NSString *s) {
        if ([s isEqualToString:weakSelf.topView.leftButton.currentTitle]) {
            ...
            addressVc.addressBackBlock = ^(NSString *addressID, NSString *addressStr, NSString *parentProID,NSString * address_Str) {
                ...
                {
                    weakSelf.n_portID = [addressID componentsSeparatedByString:@"-"][1];
                }
...
//    tableView.rowHeight = 100;
    ...
    [tableView makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.view).offset(kStatusBarHeight + kNavigationBarHeight + realH(100));
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BoatDetailViewController * boatDetailVc = [[BoatDetailViewController alloc] init];
    boatDetailVc.hidesBottomBarWhenPushed = YES;
    HomeBoatModel * model = self.dataArray[indexPath.row];
    boatDetailVc.idStr = model.shipping_id;
...
-(void)findBoatRequest
{
    ...
    parameterstring = [NSString stringWithFormat:@"\"n_port\":\"%@\",\"weight\":\"%@\",\"has_cover\":\"\",\"cover_port\":\"\",\"ship_type\":\"%@\",\"max\":\"%lu\",\"page\":\"%lu\",\"access_token\":\"%@\"",self.n_portID,self.weightStr,self.typeStr,(unsigned long)self.max,(unsigned long)self.page,[UseInfo shareInfo].access_token];
...
-(void)shipTypeRequest
{
    ...
    [XXJNetManager requestPOSTURLString:ShipType URLMethod:ShipTypeMethod parameters:nil finished:^(id result) {
        ...
        self.shaiXuanArray = [NSArray arrayWithArray:array];
    ...
    _callCenter.callEventHandler = ^(CTCall * call) {
        if ([call.callState isEqualToString:CTCallStateDisconnected]) {// Call has been disconnected
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (![viewControllerNow  isKindOfClass:[HomeViewController class]]) {   //如果不是页面XXX，则执行下面语句
