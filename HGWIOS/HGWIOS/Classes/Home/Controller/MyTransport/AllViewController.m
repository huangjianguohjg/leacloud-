    ...
    cell.transportBlock = ^(NSString *s) {
        ...
        else if ([s isEqualToString:@"支付履约保证金"])
        {
            ...
            if ([[UseInfo shareInfo].identity isEqualToString:@"船东"]) {
                bondVc.payBond = model.p_c_payment_money;
            }
            else
            {
                bondVc.payBond = model.p_s_payment_money;
            }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    myTransDetailVc.weight_num = model.weight_num;
    [self.navigationController pushViewController:myTransDetailVc animated:YES];
    
//    UploadBillViewController * billVc = [[UploadBillViewController alloc]init];
//    billVc.cargo_id = model.cargo_id;
//    billVc.titleStr = @"上传收货单";
//    [self.navigationController pushViewController:billVc animated:YES];
    
//    DiscussOtherViewController * disscussVc = [[DiscussOtherViewController alloc]init];
//    disscussVc.titleStr = @"评价对方";
//    [self.navigationController pushViewController:disscussVc animated:YES];
    
//    PayBondViewController * bondVc = [[PayBondViewController alloc]init];
//    bondVc.cargo_id = model.cargo_id;
//    bondVc.titleStr = @"111";
...
{
    NSString *parameterstring = [NSString stringWithFormat:@"\"deal_status\":\"%@\",\"max\":\"%lu\",\"page\":\"%lu\",\"access_token\":\"%@\"",@"0",(unsigned long)self.max,(unsigned long)self.page,[UseInfo shareInfo].access_token];
