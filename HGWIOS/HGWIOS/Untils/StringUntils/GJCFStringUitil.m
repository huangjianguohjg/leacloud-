...
+ (BOOL)stringIsNull:(NSString *)string
{
    ...
    if (!string || [string isKindOfClass:[NSNull class]] || [[string stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] length]== 0 || [string isEqualToString:@""]) {
        return YES;
    }else{
        return NO;
    }
...
+(NSString *)numberFromString:(NSString *)value{
    ...
    for (int  i = 0;i< value.length; i++) {
        ...
        range.location = i;
        range.length = 1;
        NSString *item = [value substringWithRange:range];
        if ([GJCFStringUitil stringJustHasNumber:item]) {
